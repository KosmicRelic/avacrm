...
// Sync basic fields across linked records
exports.syncLinkedRecordBasicFields = functions.https.onRequest((req, res) => {
  corsMiddleware(req, res, async () => {
    if (req.method === 'OPTIONS') {
      functions.logger.info('Handling OPTIONS request for CORS preflight');
      return res.status(204).send('');
    ...
    try {
      functions.logger.info('syncLinkedRecordBasicFields called', {
        method: req.method,
        headers: req.headers,
      });
      ...
      const { businessId, recordData } = body || {};

      functions.logger.info('Parsed body', {
        businessId,
        recordDocId: recordData?.docId,
        recordLinkId: recordData?.linkId,
        recordTypeOfObject: recordData?.typeOfObject
      });

      if (!businessId || !recordData || !recordData.docId || !recordData.linkId) {
        functions.logger.error('Missing required fields', { businessId, hasRecordData: !!recordData });
        return res.status(400).json({
          error: 'Missing required fields: businessId, recordData.docId, recordData.linkId',
        });
      }

      // Get the template object to determine basic fields
      const templateObjectsRef = admin.firestore().collection('businesses').doc(businessId).collection('templateObjects');
      const templateObjectsSnapshot = await templateObjectsRef.get();
      ...
      for (const doc of templateObjectsSnapshot.docs) {
        ...
        if (objectData.name === recordData.typeOfObject) {
      ...
      if (!foundObject || basicFields.length === 0) {
        functions.logger.info('No basic fields found for object:', recordData.typeOfObject);
        return res.status(200).json({
          success: true,
          message: 'No basic fields to sync',
          updatedCount: 0
        });
      }

      functions.logger.info('Found basic fields:', { count: basicFields.length, fields: basicFields.map(f => f.key) });

      // Get all records with the same linkId
      const recordsRef = admin.firestore().collection('businesses').doc(businessId).collection('records');
      const linkedRecordsQuery = recordsRef.where('linkId', '==', recordData.linkId);
      const linkedRecordsSnapshot = await linkedRecordsQuery.get();

      const batch = admin.firestore().batch();
      let updatedCount = 0;

      functions.logger.info('Found linked records:', { total: linkedRecordsSnapshot.size });

      // Update each linked record (except the one that was just updated)
      linkedRecordsSnapshot.docs.forEach((recordDoc) => {
        const existingRecordData = recordDoc.data();

        // Skip the record that triggered the update
        if (recordDoc.id === recordData.docId) {
          functions.logger.info('Skipping source record:', recordDoc.id);
          return;
        }
        ...
        basicFields.forEach(field => {
          ...
          if (recordData.hasOwnProperty(fieldKey) && recordData[fieldKey] !== existingRecordData[fieldKey]) {
            updateData[fieldKey] = recordData[fieldKey];
            functions.logger.info(`Updating field ${fieldKey} from "${existingRecordData[fieldKey]}" to "${recordData[fieldKey]}" in record ${recordDoc.id}`);
          }
        });
        ...
        if (Object.keys(updateData).length > 0) {
          ...
          const currentHistory = existingRecordData.history || [];
          const syncHistoryEntry = {
            field: 'basic_fields_sync',
            oldValue: 'Various fields updated',
            newValue: 'Synced from linked record',
            timestamp: new Date().toISOString(),
            user: recordData.lastModifiedBy || decodedToken.uid,
            action: 'sync'
          };
          ...
          updatedCount++;
          functions.logger.info('Updating linked record:', recordDoc.id);
        } else {
          functions.logger.info('No changes needed for record:', recordDoc.id);
        }
      });

      if (updatedCount > 0) {
        await batch.commit();
        functions.logger.info(`Successfully synced basic fields to ${updatedCount} linked records`);
        return res.status(200).json({
          success: true,
        ...
        functions.logger.info('No linked records needed updating');
        return res.status(200).json({
          success: true,
      ...
      functions.logger.error('syncLinkedRecordBasicFields failed', {
        error: error.message,
        stack: error.stack,
      });
      return res.status(500).json({
        error: `Failed to sync linked record basic fields: ${error.message}`
      });
    }
  });
});