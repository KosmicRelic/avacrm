...
const Window = ({ size, widget, style, onDelete, editMode, isSelected, onClick }) => {
...
const DashboardPlane = ({
  ...
  const attemptSameDashboardSwap = (indexA, indexB) => {
    ...
    const windowB = localItems[indexB];
    ...
    console.log(`Window A: size ${sizeA}, pos ${JSON.stringify(posA)}`);
    console.log(`Window B: size ${sizeB}, pos ${JSON.stringify(posB)}`);

    // Attempt direct swap by checking if each widget can fit in the other's position
    ...
    if (
      ...
      updateWidgets(dashboardId, updatedWindows.map((win) => ({
        ...win.originalWidget,
    ...
    // Fallback to group or repositioning swap
    ...
    if (groupSmall) {
      ...
      if (canPlaceWindow(localWindows[largeIndex].size, minRow, minCol, indices, null, localWindows)) {
        ...
        for (const { index, relRow, relCol } of relativePositions) {
          ...
          if (canPlaceWindow(size, newRow, newCol, [largeIndex, ...indices], null, localWindows)) {
            groupPositions.push({ index, position: { row: newRow, col: newCol } });
          } else {
            const pos = findPositionForWindow(size, [largeIndex, ...indices], null, localWindows);
            if (pos) {
      ...
      const largePos = findPositionForWindow(largeWin.size, [largeIndex, smallIndex], null, localWindows);
      const smallPos = findPositionForWindow(smallWin.size, [largeIndex, smallIndex], null, localWindows);
  ...
  const attemptCrossDashboardSwap = (
    ...
    const sourceWindows = dashboardId === sourceDashboardId ? windows : sourceWidgets.map((widget, idx) => {
      ...
      const position = widget.position || findPositionForWindow(size, [], Array(4).fill().map(() => Array(2).fill(false)), []) || { row: 0, col: 0 };
      return {
    ...
    targetWidgets.forEach((widget, idx) => {
      const size = sizeMap[widget.size] || 'small';
      const position = widget.position || findPositionForWindow(size, [], targetGrid, targetWindows) || { row: 0, col: 0 };
      targetWindows.push({
        originalWidget: widget,
        size,
        position,
    ...
    if (areaA === areaB) {
      ...
      if (
        canPlaceWindow(sizeA, posB.row, posB.col, [sourceIndex], updatedTargetGrid, sourceWindows) &&
        canPlaceWindow(sizeB, posA.row, posA.col, [targetIndex], sourceGrid, targetWindows)
      ) {
        ...
        const sourceScore = calculateScore(updatedSourceWindows);
        const targetScore = calculateScore(updatedTargetWindows);
        if (sourceScore > 80 || targetScore > 80) {
          console.log(`Swap rejected: source score ${sourceScore}, target score ${targetScore} exceed limit of 80`);
          return false;
        }
      ...
      if (groupSmall) {
        ...
        let largePosition = findPositionForWindow(largeWindows[largeIndex].size, indices, smallGrid, smallWindows);
        if (!largePosition) {
          ...
          tempWindows.forEach((win, idx) => {
            const pos = findPositionForWindow(win.size, [], tempGrid, repositionedWindows);
            if (pos) {
          ...
          if (canReposition) {
            largePosition = findPositionForWindow(largeWindows[largeIndex].size, [], tempGrid, repositionedWindows);
          }
        }
        ...
        if (largePosition) {
          ...
          for (const { index } of relativePositions) {
            ...
            const size = win.size;
            const pos = findPositionForWindow(size, [], tempGroupGrid, remainingSourceWindows.concat(groupPositions));
            if (pos) {
          ...
          if (canPlaceGroup) {
            ...
            const largeScore = calculateScore(updatedLargeWindows);
            const smallScore = calculateScore(updatedSmallWindows);
            if (largeScore > 80 || smallScore > 80) {
              console.log(`Group swap rejected: large score ${largeScore}, small score ${smallScore} exceed limit of 80`);
              return false;
            }
        ...
        const largePos = findPositionForWindow(largeWin.size, [smallIndex], smallGrid, smallWindows);
        const smallPos = findPositionForWindow(smallWin.size, [largeIndex], largeGrid, largeWindows);
        if (largePos && smallPos) {
          ...
          const smallScore = calculateScore(updatedSmallWindows);
          if (largeScore > 80 || smallScore > 80) {
            console.log(`Fallback swap rejected: large score ${largeScore}, small score ${smallScore} exceed limit of 80`);
            return false;
          }
          ...
          updateWidgets(smallDashboardId, updatedSmallWindows.map((win) => win.originalWidget));
          if (dashboardId === largeDashboardId) setWindows(updatedLargeWindows);
          else if (dashboardId === targetDashboardId) setWindows(updatedSmallWindows);
          return true;
        } else {
