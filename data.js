...
// Initial state data (in a real app, this might be passed as a parameter or imported)
const initialState = {
  sheets: {
    allSheets: [
      {
...
export const BusinessSignUp = async ({
  ...
  try {
    ...
    // Store user data in Firestore /users
    const userData = {
      uid: user.uid,
      email: user.email,
      businessName,
    ...
    // Create business data in Firestore /businesses
    ...
    // Sheets: Save allSheets and dynamically generate structure
    initialState.sheets.allSheets.forEach((sheet) => {
      const sheetRef = doc(db, 'businesses', businessId, 'sheets', sheet.id);
      batch.set(sheetRef, sheet);
    });
    const structure = initialState.sheets.allSheets.map((sheet) => ({ sheetName: sheet.sheetName }));
    const structureRef = doc(db, 'businesses', businessId, 'sheetsStructure', 'structure');
    batch.set(structureRef, { structure });

    // Cards
    initialState.cards.forEach((card) => {
      const cardRef = doc(db, 'businesses', businessId, 'cards', card.id);
      batch.set(cardRef, card);
    });

    // Card Templates
    initialState.cardTemplates.forEach((template) => {
      const templateRef = doc(db, 'businesses', businessId, 'cardTemplates', template.name);
      batch.set(templateRef, template);
    });

    // Metrics
    initialState.metrics.forEach((category) => {
      const metricRef = doc(db, 'businesses', businessId, 'metrics', category.category);
      batch.set(metricRef, category);
    });

    // Dashboards
    initialState.dashboards.forEach((dashboard) => {
      const dashboardRef = doc(db, 'businesses', businessId, 'dashboards', dashboard.id);
